...
export default function Navbar() {
  ...
  return (
    <nav
      className={`bg-panel/95 backdrop-blur-lg shadow-token sticky top-0 z-50 border-b border-border ${
        mobileMenu ? "min-h-screen" : "min-h-[64px]"
      }`}
      role="navigation"
    ...
    >
      <div
        className={`w-full px-4 sm:px-6 lg:px-8 ${
          mobileMenu
            ? "min-h-screen flex flex-col items-start py-6"
            : "min-h-[64px] flex items-center justify-between"
        }`}
      >
        {/* TOP ROW */}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-6 text-text dark:text-muted">
              ...
              {isGuest && (
                <>
                  {!hideHome && (
                    <Link to="/" className="hover:text-accent-600 transition">
                      Home
                    </Link>
                  )}
                  <Link to="/about" className="hover:text-accent-600 transition">
                    About
                  </Link>
                  <Link
                    to="/gallery"
                  ...
                  <Link to="/team" className="hover:text-accent-600 transition">
                    Team
                  </Link>
                  <Link
                    to="/packages"
                  ...
                  <div className="flex items-center gap-3">
                    <Link
                      to="/login"
                      className="px-4 py-2 bg-accent-600 text-white rounded hover:bg-accent-500 shadow"
                    >
                      Login / Sign Up
                    </Link>
                    <Link
              ...
              {isUser && (
                <>
                  <Link
                    to="/book-service"
                    className="px-4 py-2 bg-accent-600 text-white rounded hover:bg-accent-500"
                  >
                    Book Service
                  </Link>
                  ...
                  {user && (
                    <div className="relative" ref={profileRef}>
                      ...
                      >
                        <div
                          className="w-10 h-10 bg-accent-600 flex items-center justify-center text-white rounded-full font-bold text-lg"
                          aria-hidden
                        >
                      ...
                      <AnimatePresence>
                        {profileMenu && (
                          <motion.div
                            ...
                            exit={{ opacity: 0, y: -8 }}
                            className="absolute right-0 mt-3 bg-panel w-48 rounded-xl shadow-lg border border-border p-3 text-sm"
                            role="menu"
                          ...
                          >
                            <Link
                              ...
                              className="block px-3 py-2 hover:bg-panel/80 rounded"
                              role="menuitem"
                            >
                              Dashboard
                            </Link>
              ...
              {/* For admins: show admin-specific links (includes Add Team) */}
              {isAdmin && (
                <>
                  {!hideHome && (
                    <Link to="/" className="hover:text-accent-600 transition">
